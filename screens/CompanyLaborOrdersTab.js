...
export default function CompanyLaborOrdersTab() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const snapshot = await getDocs(collection(firestore, 'laborOrders'));
        ...
        console.error('Error loading labor orders:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const acceptOrder = async (orderId) => {
    try {
      await updateDoc(doc(firestore, 'laborOrders', orderId), {
        status: 'Accepted',
  ...
  return (
    <View style={styles.container}>
      {orders.length === 0 ? (
        <Text style={styles.empty}>🎉 No pending labor orders right now.</Text>
      ) : (
        <FlatList
          data={orders}
          ...
          renderItem={({ item }) => (
            <View style={styles.card}>
              ...
              <Text style={styles.details}>
                Start Price: {item.startPrice?.toLocaleString()} soums
              </Text>
              <Pressable
